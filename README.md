...
| `adata.var_names` | Gene symbols | No manual gene matching needed — PRISM aligns your data to its trained ~10,457-gene panel automatically (genes it expects but you don't have are zero-filled; genes you have that it doesn't use are simply ignored). It logs how much overlap there was (`Found X% reference vars in query data`) during the scVI embedding step, just so you can sanity-check it — this doesn't block anything, but a low percentage means more of the embedding is running on zero-filled genes, so treat results a bit more cautiously in that case. |
